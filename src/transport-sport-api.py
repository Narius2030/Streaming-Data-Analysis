...
def transport(topic):
    prod_tasks = [
        Producer(topic=topic, function=request_top_soccer, key='top_soccer'),
    ]
    
    cons_tasks = [
        Consumer(topic=topic, group_id='sport-api', path='./logs', function=write_logs),
        Consumer(topic=topic, group_id='sport-api', path='./logs', function=write_logs)
    ]
